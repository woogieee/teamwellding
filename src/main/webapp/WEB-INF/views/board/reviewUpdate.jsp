...
$(document).ready(function(){
	<c:choose>
	   ...
	   $("#btnFileDelete").on("click", function(){
	      ...
	      $.ajax({
	         type: "POST",
	         url: "/board/ReviewdeleteFileProc",
	         data: {
	...
	<div class="col-lg-10">
		...
		<div class="containerfW">
		   ...
		   <form name="updateForm" id="updateForm" method="post" enctype="multipart/form-data">
		      ...
		      <input type="text" name="bTitle" id="bTitle" maxlength="100" style="ime-mode:active;" class="form-control mb-2" placeholder="제목을 입력해주세요." required value="${wdReview.RTitle}" /> 
		      ...
		      별점을 선택해 주세요 : <select id="starScore" name="starScore">
			      ...
			      <div style="margin-bottom:0.3em;">[첨부파일 : <span id="fileNameSpan">${wdReview.reviewFile.rFileOrgName}</span>]&nbsp;
		      ...
		      <br>
			<input type="hidden" name="RSeq" value="${wdReview.RSeq}" />
		    <input type="hidden" name="searchValue" value="${searchValue}" />
		    <input type="hidden" name="curPage" value="${curPage}" />
		      
		    <input type="hidden" name="oldTitle" id="oldTitle" value="${wdReview.RTitle}">
		    <input type="hidden" name="oldContent" id="oldContent" value="${wdReview.RContent}">
			

		   </form>

			<form name="bbsForm" id="bbsForm" method="post">
			   <input type="hidden" name="bSeq" value="${wdReview.RSeq}" />
			   <input type="hidden" name="searchValue" value="${searchValue}" />
			   <input type="hidden" name="curPage" value="${curPage}" />
			</form>
